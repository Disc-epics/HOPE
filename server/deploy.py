...
if __name__ == '__main__':
    ...
    # copy them all
    conn.storeFile('disc', 'public_html/django.cgi', open('django.cgi'))
    conn.storeFile('disc', 'public_html/manage.py', open('manage.py'))
        ...
        for name in filenames:
            if os.path.splitext(name)[1] != '.pyc':
                conn.storeFile('disc', os.path.join(
                    'public_html', dp, name), open(os.path.join(dp, name)))
